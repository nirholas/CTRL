...
* where `folderName` is the Folder Name the file should be saved in (available: `Downloads` and `Apps`), and `fileName` is the File Name the file should be saved as. This **should not contain** a file extension, learn how to remove file extensions with CTRL.js.
...
* where `UID` is an Identification of a File, usually it is a 6 Characters long string assigned to every single file in CTRL. You may obtain it using the following function (getFileNamesByFolder)
